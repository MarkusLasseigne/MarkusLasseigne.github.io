...
describe("Test Guru", function(){
	...
	it("Functions can access/modify variables in parent scope.", function(){
		...
		yay(); expect(outside_the_function === 'can you see me?').to.be.true;
	});
	...
	it("A functions local scope is not available in an outer scope.", function(){
		...
		if(this.kix !== undefined){has_kix=kix;}
		else{has_kix="i prefer cheerios";}
		expect(has_kix === 'i prefer cheerios').to.be.true;
	});

	it("Functions don't have access to eachothers scope", function(){
		function yay(){this.from_yay="i'm inside yay;";}

		function foo(){
			var in_foo="i'm in foo";
			if(this.from_yay !== undefined){in_foo=this.from_yay;}
	...
	it("Inner scope variables have preference over outter scope variables with the same name.", function(){
		...
		yay(); expect(peanuts === 300).to.be.true;
	});

	it("Variables created with var in a funtion are re-created each time", function(){
		function yay(){
			if(this.counter !== undefined){counter=counter + 1;}
			else{var counter=10;}
		}
		yay(); expect(this.counter === undefined).to.be.true;
		yay(); expect(this.counter === undefined).to.be.true;
		yay(); expect(this.counter === undefined).to.be.true;
	});
	...
	it("Functions retain outer scope references between calls.", function(){
		...
		yay(); expect(im_outside === 14).to.be.true;
		yay(); expect(im_outside === 15).to.be.true;
	});
	...
	it("We can do goofy stuff with outer scope", function(){
		...
		function yay(){name +=hello;}
		yay(); expect(name === 'greg').to.be.true;
		yay(); expect(name === 'greggreg').to.be.true;
		yay(); expect(name === 'greggreggreg').to.be.true;
	});

	it("We can pass functions to other functions and then run them.", function(){
		var im_outter=10;
		function yay(){im_outter /=5;}
		function something(whatever){
			im_outter *=20;
			whatever();
		}
		something(yay);	expect(im_outter === 40).to.be.true;
	});

	it("We can get crazy with returns.", function(){
		function yay(){return " is dog";}

		function foo(whatever){return "hello, this" + whatever();}
